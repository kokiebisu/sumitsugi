...
```
.github/workflows/
├── ci.yml                      # CI: lint + types + unit tests (on PR/push main)
├── e2e-tests.yml               # E2E: Playwright (smart label filtering)
├── autonomous-developer.yml    # Every 4h: pick task → Claude Code → PR
...
| CI | PR/push | ESLint + TypeScript + unit tests |
| E2E | PR labels | Playwright (smart filtering) |
