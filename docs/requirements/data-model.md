...
| isProCoordinated      | boolean             | -    | プロにコーディネートされた物件か（デフォルト: false）。~~座談会#15でMVPデータ収集用に決定~~ → [座談会#16で撤回](../requirements/meetings/2026-02-12-product-meeting-16-delta.md)。UIチェックボックスはMVPから削除。DBカラムはPhase 2用に維持。表示機能F-901〜F-903はP2 |
