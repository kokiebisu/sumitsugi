...
export class PaymentPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.paymentFlowSection = page.locator(
      'section:has(h2:text("お支払いの流れ")), [class*="Card"]:has-text("お支払いの流れ")'
    );
    this.feeBreakdownSection = page.locator(
      'section:has(h2:text("費用の内訳")), [class*="Card"]:has-text("費用の内訳")'
    );
    this.paymentFormCard = page.locator(
      '[class*="Card"]:has-text("申込金のお支払い")'
    );
    this.trustIndicators = page.locator(
      '[class*="Card"]:has-text("安全な決済")'
    );

    // Payment steps
    this.applicationFeeStep = page.locator(
      '[class*="rounded-lg"]:has-text("申込金のお支払い（現在のステップ）")'
    );
    this.depositStep = page.locator(
      '[class*="rounded-lg"]:has-text("デポジット（30%）")'
    );
    this.remainingStep = page.locator(
      '[class*="rounded-lg"]:has-text("残金（70%）")'
    );

    // Fee breakdown
    this.handoverFeeTotal = page
      .locator('text=/引き継ぎ費用\\s+合計/')
      .locator('..');
    this.sellerReceivesAmount = page
      .locator('text=前の住人が受け取る金額')
      .locator('..')
      .locator('span:last-child');
    this.additionalCleaningFee = page
      .locator('text=追加クリーニング費用')
      .locator('..')
      .locator('span:last-child');
    this.landlordIncentive = page
      .locator('text=大家さんインセンティブ')
      .locator('..')
      .locator('span:last-child');
    this.platformFee = page
      .locator('text=プラットフォーム手数料')
      .locator('..')
      .locator('span:last-child');

    // Application fee form
    this.applicationFeeFormCard = page
      .locator('[class*="Card"]:has-text("申込金のお支払い")')
      .first();
    this.applicationFeeFormTitle = this.applicationFeeFormCard.locator(
      '[class*="CardTitle"]'
    );
    this.warningBanner = page.locator(
      '[class*="bg-yellow"]:has-text("重要：申込金について")'
    );
    this.applicationFeeAmount = page.locator('text=/申込金.*円/');
    this.stripePaymentElement = page.locator(
      '#payment-element, [class*="StripeElement"], [id*="card-element"]'
    );
    this.submitButton = page.locator(
      'button[type="submit"]:has-text("円を支払う")'
    );
    ...
    this.escrowProtectionBadge = page
      .locator('text=エスクロー保護')
      .locator('..');
  ...
  async getApplicationFeeAmount(): Promise<string | null> {
    const amountElement = this.page.locator(
      'text=/\\d{1,3}(,\\d{3})*円を支払う/'
    );
  ...
  async isStripeElementLoaded(): Promise<boolean> {
    ...
    const paymentElement = this.page.locator(
      '[id*="payment-element"], [class*="StripeElement"]'
    );
  ...
  async fillTestCardDetails(cardNumber: string = '4242424242424242') {
    ...
    // Fill card number
    await stripeFrame
      .locator('[name="cardnumber"], [placeholder*="card number" i]')
      .fill(cardNumber)
      .catch(() => {
    ...
    await stripeFrame
      .locator('[name="exp-date"], [placeholder*="MM" i]')
      .fill('12/30')
      .catch(() => {});
    ...
    await stripeFrame
      .locator(
        '[name="postal"], [placeholder*="postal" i], [placeholder*="ZIP" i]'
      )
      .fill('12345')
      .catch(() => {});
  }
  ...
  async waitForPaymentCompletion(timeoutMs: number = 30000) {
    await this.processingIndicator
      ...
      .catch(() => {});
    await this.processingIndicator.waitFor({
      state: 'hidden',
      timeout: timeoutMs,
    });
  }
