...
export class ListingPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.createButton = page.locator(
      'a[href="/listing/new"] button, a:has-text("リスティングを作成")'
    );
    this.emptyStateTitle = page.locator(
      'h1:has-text("最初のリスティングをはじめよう")'
    );
    this.listingCards = page.locator('.group.relative.bg-white.rounded-2xl');
    // Listing count is shown in a badge next to "リスティング" tab
    this.listingCount = page.locator(
      'button:has-text("リスティング") span.rounded-full'
    );
    this.newListingButton = page.locator('a[href="/listing/new"]');
    this.scrollingImages = page.locator(
      '[style*="animation: scrollLeft"], [style*="animation: scrollRight"]'
    );
  }
  ...
  async getListingCardData(index: number) {
    ...
    const statusBadge = await card
      .locator('span[class*="rounded-full"]')
      .textContent();
    const dateInfo = await card
