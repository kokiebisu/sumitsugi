...
export class PropertyDetailPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.furnitureSection = page.locator('section:has(h2:text("引き継ぎ対象の大型家具"))')
  ...
  async getLayout(): Promise<string | null> {
    ...
    await layoutSection.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {})
  ...
  async getOccupancy(): Promise<string | null> {
    ...
    await occupancySection.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {})
