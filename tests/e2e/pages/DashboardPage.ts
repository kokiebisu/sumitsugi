...
export class DashboardPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.pageDescription = page.locator(
      'text=申し込んだ暮らしの引き継ぎ状況を確認できます'
    );
    this.emptyState = page.locator(
      '[class*="border"]:has-text("申し込んだ暮らしがありません")'
    );
    ...
    this.propertyTitles = page.locator(
      '[class*="rounded-xl"] a[href^="/listings/"]'
    );
    ...
    this.completedStatusBadge = page.locator(
      '[class*="bg-green"]:has-text("完了")'
    );
    this.rejectedStatusBadge = page.locator(
      '[class*="bg-red"]:has-text("お断り")'
    );
    this.cancelledStatusBadge = page.locator(
      '[class*="bg-red"]:has-text("キャンセル")'
    );
    ...
    this.viewingCompletedStep = page
      .locator('p:text-is("内見完了")')
      .locator('..');
    ...
    // Next action
    this.nextActionMessage = page.locator(
      '[class*="rounded-lg"][class*="p-4"] p'
    );
  }
  ...
  async getInquiryCount(): Promise<number> {
    ...
    const cards = await this.page
      .locator(
        '[class*="rounded-xl"][class*="border"]:has(a[href^="/listings/"])'
      )
      .all();
    return cards.length;
  }
