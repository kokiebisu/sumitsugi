...
export class NewListingPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // Common elements
    this.stepIndicator = page.locator('p.text-base.font-medium:has-text("ステップ")')
    this.stepTitle = page.locator('h1[class*="text-\\[48px\\]"]')
    this.nextButton = page.locator('button:has-text("次へ"), button:has-text("公開する"), button:has-text("登録する")')
    ...
    // Step 2
    this.addPhotoButton = page.locator('button:has-text("写真を追加"), button:has(svg.lucide-plus)')
    this.photoUploadDialog = page.locator('.fixed.inset-0.z-50:has(h2:has-text("写真をアップロード"))')
    ...
    this.layoutOptions = page.locator('button:has-text("1R"), button:has-text("1K"), button:has-text("1LDK")')
    this.occupantsOptions = page.locator('button:has-text("1人"), button:has-text("2人")')

    // Step 5
    this.viewingDatePicker = page.locator('button:has-text("内見可能日を選択")')
    this.moveInDatePicker = page.locator('button:has-text("引き継ぎ可能日を選択")')

    // Step 6
    this.furnitureOptions = page.locator('button:has(svg.lucide-bed-double), button:has(svg.lucide-sofa)')
  ...
  async getSelectedFurniture(): Promise<string[]> {
    const selected = await this.page.locator('button[class*="border-foreground"]:has(svg)').all()
    const items: string[] = []
