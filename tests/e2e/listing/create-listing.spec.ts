...
test.describe('Create Listing - Step Navigation', () => {
  ...
  test('should start at step 1 with introduction', async ({ newListingPage }) => {
  ...
  test('should have save and exit button', async ({ newListingPage }) => {
    ...
    await expect(newListingPage.saveAndExitButton).toContainText('保存して終了')
  })

  test('should navigate to step 2 when clicking next', async ({ newListingPage }) => {
    await newListingPage.goto()
  ...
  test('should navigate back to step 1 when clicking back from step 2', async ({ newListingPage }) => {
    await newListingPage.goto()
  ...
  test('should show progress bar with 7 segments', async ({ newListingPage, page }) => {
    await newListingPage.goto()
    ...
    const progressSegments = await page.locator('footer div.flex:first-child > div').count()
...
test.describe('Create Listing - Step 2: Photos', () => {
  ...
  test('should open upload dialog when clicking add photo', async ({ newListingPage }) => {
    await newListingPage.goto()
    await newListingPage.goToNextStep()
  ...
  test('next button should be disabled without 5 photos', async ({ newListingPage }) => {
    await newListingPage.goto()
    await newListingPage.goToNextStep()
...
test.describe('Create Listing - Step 4: Property Info', () => {
  ...
  test('should have rent input field with placeholder', async ({ newListingPage, page }) => {
    await newListingPage.goto()
  ...
  test('should have layout selection options', async ({ newListingPage, page }) => {
    await newListingPage.goto()
...
test.describe('Create Listing - Save and Exit', () => {
  ...
  test('should save as draft and redirect to listing page', async ({ newListingPage, page }) => {
    await newListingPage.goto()
