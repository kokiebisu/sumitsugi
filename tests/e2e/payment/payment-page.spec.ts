...
test.describe('Payment Page @payment', () => {
  ...
  test('should display payment page with correct title', async ({
    paymentPage,
  }) => {
  ...
  test('should display trust indicators', async ({
  ...
  test('should display non-refundable warning', async ({
    paymentPage: _paymentPage,
  }) => {
  ...
  test('should display correct application fee amount', async ({
    paymentPage: _paymentPage,
  }) => {
  ...
  test('should have terms and privacy links', async ({
    paymentPage: _paymentPage,
    page,
  }) => {
  ...
  test('should navigate to terms page when clicked', async ({
    paymentPage: _paymentPage,
    page,
  }) => {
    void _paymentPage; // Used for fixture setup
  ...
  test('should navigate to privacy page when clicked', async ({
    paymentPage: _paymentPage,
    page,
  }) => {
    void _paymentPage; // Used for fixture setup
...
test.describe('Payment Page - Fee Breakdown Verification @payment', () => {
  ...
  test('should show application fee as current step', async ({ page }) => {
    ...
    const applicationFeeStep = page.locator(
      '[class*="border-\\[#FF5A5F\\]"], [class*="border-coral"]'
    );
  ...
  test('should show deposit and remaining steps as inactive', async ({
    page,
  }) => {
    const propertyId = testData.properties.bohemian;
    ...
    const depositStep = page.locator(
      '[class*="opacity-60"]:has-text("デポジット")'
    );
    await expect(depositStep).toBeVisible();

    // Remaining step should be grayed out
    const remainingStep = page.locator(
      '[class*="opacity-60"]:has-text("残金")'
    );
