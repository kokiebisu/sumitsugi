...
test.describe('Payment Flow - User Journey @payment', () => {
  ...
  test('user can navigate from property to payment page', async ({
    page,
    homePage,
  }) => {
    ...
    await expect(
      page.locator('text=引き継ぎ費用のお支払い手続き')
    ).toBeVisible();
  });
  ...
  test('authenticated user sees payment form loaded', async ({ page }) => {
    ...
    await page.waitForSelector('[class*="Card"]:has-text("申込金のお支払い")', {
      timeout: 10000,
    });
...
test.describe('Payment Flow - Pre-payment Checks @payment', () => {
  ...
  test('shows next steps information', async ({ page }) => {
    ...
    await expect(
      page.locator('text=前の住人とメッセージでやり取りを開始できます')
    ).toBeVisible();
  });
...
test.describe('Payment Flow - Cross-Browser @payment', () => {
  ...
  test('payment page renders correctly', async ({
    page,
  ...
  test('fee breakdown displays correctly', async ({
    page,
    browserName: _browserName,
  }) => {
