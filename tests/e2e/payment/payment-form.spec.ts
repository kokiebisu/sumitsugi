...
test.describe('Payment Form - Stripe Elements @payment', () => {
  ...
  test('should display application fee amount in form', async ({
    paymentPage,
    page,
  }) => {
    await paymentPage.waitForPaymentFormLoad();
  ...
  test('should show warning about non-refundable payment', async ({
    paymentPage,
    page,
  }) => {
    await paymentPage.waitForPaymentFormLoad();
  ...
  test('should have disabled submit button initially', async ({
    paymentPage,
  }) => {
    await paymentPage.waitForPaymentFormLoad();
...
test.describe('Payment Form - Stripe Integration @payment', () => {
  ...
  test('should load Stripe payment element', async ({ paymentPage, page }) => {
    ...
    const stripeFrame = page.locator(
      'iframe[name*="stripe"], iframe[src*="stripe"], iframe[name*="__privateStripeFrame"]'
    );
    ...
    const paymentElement = page.locator(
  ...
  test('should display submit button with correct amount', async ({
    paymentPage,
    page,
  }) => {
    await paymentPage.waitForPaymentFormLoad();
...
test.describe('Payment Form - Error Handling @payment', () => {
  ...
  test('should display error when seller has no Stripe account', async ({
    page: _page,
  }) => {
    ...
    const errorMessage = _page.locator(
  ...
  test('should show user-friendly error message for API failures', async ({
    page: _page,
  }) => {
...
test.describe('Payment Form - Mobile Responsiveness @payment', () => {
  ...
  test('should be usable on mobile viewport', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'tests/e2e/artifacts/payment-form-mobile.png',
      fullPage: true,
    });
  });
...
test.describe('Payment Form - Tablet Responsiveness @payment', () => {
  ...
  test('should display properly on tablet viewport', async ({ page }) => {
    ...
    await expect(
      leftColumn.or(page.locator('[class*="Card"]:has-text("お支払いの流れ")'))
    ).toBeVisible();

    // Take tablet screenshot
    await page.screenshot({
      path: 'tests/e2e/artifacts/payment-form-tablet.png',
      fullPage: true,
    });
  });
});
