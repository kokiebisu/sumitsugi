...
test.describe('Inquiry Flow - Unauthenticated User @inquiry @critical', () => {
  ...
  test('should show inquiry button on property detail page', async ({
    page,
  }) => {
    ...
    const inquiryButton = page.locator(
      'button:has-text("この暮らしを引き継ぐ")'
    );
    await expect(inquiryButton).toBeVisible();
  });

  test('should show signup dialog when clicking inquiry button while not logged in', async ({
    page,
    authPage,
  }) => {
    await page.goto(`/listings/${testData.properties.dj}`);
  ...
  test('should allow direct access to inquiry page', async ({ page }) => {
    ...
    await expect(
      page.locator('h1:has-text("この暮らしに興味がある")')
    ).toBeVisible();
  });
  ...
  test('should show property summary on inquiry page', async ({ page }) => {
    ...
    const propertyImage = page
      .locator('img[alt*="DJ"], img[alt*="Studio"]')
      .first();
  ...
  test('should trigger signup dialog when submitting without authentication', async ({
    page,
    authPage,
  }) => {
    await page.goto(`/listings/${testData.properties.dj}/inquiry`);
  ...
  test('should have back link to property detail', async ({ page }) => {
    ...
    await expect(backLink).toHaveAttribute(
      'href',
      `/listings/${testData.properties.dj}`
    );
  });
});
...
test.describe('Inquiry Flow - Authenticated User @inquiry @auth @critical', () => {
  ...
  test('should navigate to inquiry form after authentication', async ({
    page,
  }) => {
    await page.goto(`/listings/${testData.properties.dj}`);
  ...
  test('should pre-fill name and email for logged-in user', async ({
    page,
  }) => {
    await page.goto(`/listings/${testData.properties.dj}/inquiry`);
  ...
  test('should show user info banner when logged in', async ({ page }) => {
    ...
    const userBanner = page.locator(
      `text=${testData.users.testUser.name} として申し込み`
    );
    await expect(userBanner).toBeVisible();
  });

  test('should successfully submit inquiry when authenticated', async ({
    page,
  }) => {
    await page.goto(`/listings/${testData.properties.dj}/inquiry`);
    ...
    await page
      .locator('textarea#reason')
      .fill(
        'DJ機材がそのまま使える環境を探していました。ぜひ引き継がせていただきたいです。'
      );
    await page
      .locator('textarea#questions')
      .fill('機材のメンテナンス状況を教えていただけますか？');
  ...
  test('submit button should show correct text when authenticated', async ({
    page,
  }) => {
    await page.goto(`/listings/${testData.properties.dj}/inquiry`);
...
test.describe('Inquiry Flow - Edge Cases @inquiry @extended', () => {
  test('should show 404 for non-existent property inquiry', async ({
    page,
  }) => {
