...
test.describe('Browse Properties - Home Page @properties @critical @smoke', () => {
  ...
  test('should display the home page with header and footer', async ({
    homePage,
    page,
  }) => {
  ...
  test('should display property cards with essential information', async ({
    homePage,
    page,
  }) => {
  ...
  test('should navigate to property detail when clicking a card', async ({
    homePage,
    page,
  }) => {
    const propertyCount = await homePage.getPropertyCount();
  ...
  test('should show multiple properties per district section', async ({
    homePage,
  }) => {
    const districts = await homePage.getDistrictTitles();
    ...
    const propertiesInDistrict =
      await homePage.getPropertiesInDistrict(firstDistrict);

    // Should have at least one property
    expect(propertiesInDistrict.length).toBeGreaterThan(0);
  });

  test('property cards should be horizontally scrollable', async ({
    homePage,
    page,
  }) => {
    const districts = await homePage.getDistrictTitles();
...
test.describe('Browse Properties - Heart/Favorite @properties @extended', () => {
  test('should toggle heart icon when clicking favorite button', async ({
    homePage,
    page,
  }) => {
    await homePage.goto();
...
test.describe('Browse Properties - Image Carousel @properties @extended', () => {
  test('should navigate between images using arrow buttons', async ({
    homePage,
    page,
  }) => {
    await homePage.goto();
    ...
    const nextButton = firstCard.locator(
