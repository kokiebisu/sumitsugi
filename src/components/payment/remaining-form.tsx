...
import { createRemainingPayment } from '@/app/actions/payment';
...
function RemainingFormContent({
  ...
  RemainingFormProps,
  'propertyId' | 'remainingAmount' | 'onSuccess' | 'onError'
>) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `${window.location.origin}/properties/${propertyId}/payment?step=remaining&status=success`,
        },
      });
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-950/30">
        ...
        <div className="space-y-1 text-sm">
          ...
          <p className="text-blue-800 dark:text-blue-200">
            残金もエスクローに預けられ、引き渡し確認後に前の住人に送金されます。お支払い後、引き渡しの確認をお待ちください。
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center pb-2 border-b">
          <span className="font-semibold">残金（70%）</span>
      ...
      >
        {isProcessing ? (
          ...
          `${remainingAmount.toLocaleString()}円を支払う`
        )}
      </Button>
