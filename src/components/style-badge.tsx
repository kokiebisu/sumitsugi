...
const styleConfig = {
  ...
  industrial: { icon: Hammer, label: "インダストリアル", color: "text-gray-700" },
  ...
  eclectic: { icon: Sparkles, label: "エクレクティック", color: "text-pink-600" },
}
...
export function StyleBadge({ style, size = "md" }: StyleBadgeProps) {
  ...
  return (
    ...
    >
      ...
      <span className={`${textSizeClasses[size]} font-medium ${config.color}`}>{config.label}</span>
    </div>
  )
}
