...
export function LocationPicker({
  ...
  const handleAddressChange = useCallback((newAddress: JapaneseAddress) => {
  ...
  const handleMouseUp = () => {
    if (isDragging && containerRect) {
      ...
      const newLat = markerPosition.lat - LAT_RANGE * (markerOffset.y / height);
      const newLng = markerPosition.lng + LNG_RANGE * (markerOffset.x / width);
            ...
            <div key={index} className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => {
                    const newStations = stations.map((s, i) =>
                      i === index ? { ...s, walkingMinutes: e.target.value } : s
                    );
                    onStationsChange(newStations);
                  }}
